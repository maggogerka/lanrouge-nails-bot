# Референсные изображения записи v0.3.1

## Данные

`AppointmentReferenceMedia` хранит `appointment_id`, Telegram `file_id`, `file_unique_id`,
тип, позицию, загрузившего пользователя, `created_at`, `expires_at`, `deleted_at` и
безопасные счётчики попыток очистки. Бинарные изображения, caption и полный клиентский
комментарий в таблице не хранятся. Бот не скачивает оригиналы на VPS и не создаёт
...
Клиент читает или добавляет референсы только своей записи и только до
`start_at - booking_reference_edit_deadline_hours`. Явное удаление собственных
референсов доступно независимо от этого срока. Администратор получает их через
защищённый service use case. Callback ID не является подтверждением владения.

Очистка обнуляет оба Telegram ID и выставляет `deleted_at`, сохраняя минимальную строку
для аудита и целостности Appointment. Референсы никогда не публикуются в Portfolio
автоматически. Полные правила сроков, ручной запуск и мониторинг описаны в
