...
Статус: проектное решение Этапа 0. Документ описывает целевую архитектуру MVP до начала реализации.
...
| `NotificationJob` | Уникальность `(appointment_id, recipient_user_id, notification_type)`; индексы `(status, scheduled_at)`; attempts, lease и ошибка ограниченной длины. |
...
- Черновик `docs/privacy.md` в следующем этапе будет техническим описанием, а не юридической консультацией.
