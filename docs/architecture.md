...
Для составной рассылки message ID успешно отправленной media-фазы сохраняется до следующего
Telegram-вызова. Повтор после сохранённого checkpoint не отправляет media повторно. Это сужает,
но не устраняет аварийное окно: если процесс завершится после ответа Telegram, но до commit
checkpoint, повтор может создать дубль.
