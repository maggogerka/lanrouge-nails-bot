...
from app.database.models import ClientNote, ClientTag, ConsentHistory, User, UserClientTag
...
class CrmRepository:
    ...
    async def list_client_tags(self, user_id: int) -> list[ClientTag]:
        result = await self._session.scalars(
            ...
            .where(UserClientTag.user_id == user_id)
    ...
    async def assign_tag(self, *, user_id: int, tag_id: int, assigned_by: int) -> bool:
        statement = (
            ...
            .values(user_id=user_id, tag_id=tag_id, assigned_by=assigned_by)
    ...
    async def remove_tag(self, *, user_id: int, tag_id: int) -> bool:
        result = await self._session.execute(
            ...
            .where(UserClientTag.user_id == user_id, UserClientTag.tag_id == tag_id)
            .returning(UserClientTag.user_id)
        )
        return result.scalar_one_or_none() is not None

    async def get_note(self, note_id: int, *, for_update: bool = False) -> ClientNote | None:
        statement = select(ClientNote).where(ClientNote.id == note_id)
        if for_update:
            statement = statement.with_for_update()
        return (await self._session.scalars(statement)).one_or_none()
